 ...
 <div class="main">
      ...
      <v-container fluid style="background-color:#FCD675;" >
        <v-layout row wrap text-md-center>
            <v-flex  xs2 md2 sm2>
            <router-link  :to="{ name: 'workinghours', params: {} }">  <font-awesome-icon class="back-link" large   :icon="['fas','arrow-left']"/></router-link>
           </v-flex>
           <v-flex xs8 md8 sm8>
      ...
      <v-container grid-list-xs,sm,md,lg,xl>
        <v-form ref="form1" v-model="valid" lazy-validation @submit.prevent="saveBarberWorkingHours">
...
//require('../../../../../node_modules/anypicker/dist/anypicker.min.css')
  export default {
    ...
    methods: {
        ...
        saveBarberWorkingHours(){
          ...
          if (this.$refs.form1.validate()) {
                ...
                this.loader = true
                this.$store.dispatch('saveBarberWorkingHours',{
                    start_at: this.time,
                    close_at: this.time2,
                ...
                .then(response => {
                  ...
                  if(response.data.status!=undefined && response.data.status=='Token is Expired')
                  {
                    this.$router.push({ name: 'login', params: { } })
                     ...
                     this.$router.push({ name: 'SettingsWorkingHours', params: { } })
                  }
                })
                .catch(error=>{
                  this.updateBtn = true
    ...
    mounted(){
      ...
      {
          this.$store.dispatch('getSingleBarberWorkingHour',{
              hour_id: this.$route.params.hourId
          })
          .then(response => {
