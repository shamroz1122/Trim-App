   ...
   <Loader v-show="loader" />
      <v-container fluid style="background-color:#FCD675;" >
        <v-layout row wrap text-xs-center>
           ...
           <v-flex xs8 md8 sm8>
             <span class="find-text"> Services </span>
           </v-flex>
        </v-layout>
      </v-container>
              ...
              <div v-for="(service,index) in services" :key="index">
                <v-layout  justify-center row wrap  class="pt-3">
                   ...
                   <v-flex xs2 md2 sm3>
                      ...
                      v-model = "service.service_status=='true'?true:false"
                      ></v-checkbox>
                    </v-flex>
                    
                    <v-flex xs9 md8 sm4 class="my-2">

                       <span class="day"> {{ service.name }} </span>
  ...
  export default {
    ...
    methods:{
      statusOption(a,service_id){
        ...
        this.$store.dispatch('changeBarberServiceStatus',{
    ...
    this.loader = true
    this.$store.dispatch('getbarberServices',{
        user_id: this.$store.getters.loggedInUserId
    })
    .then(response => {
      this.loader = false
