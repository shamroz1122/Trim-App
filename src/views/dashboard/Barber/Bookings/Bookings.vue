		...
		<Loader v-show="loader" />
		  <v-container fluid style="background-color:#FCD675;">
			<v-layout row wrap justify-center text-xs-center>
				<v-flex xs8 offset-xs2 md8 offset-md2 sm8 offset-sm2>
				...
				<v-flex xs12 md10 sm10>
					<v-layout row wrap class="pb-2 pt-2">
						...
						<v-flex xs7 md2 sm2>
							...
							<p class="mins">
								{{	calenderBooking.service_name }} <span style="font-weight:bold !important;"> &#8364;{{calenderBooking.payment}} </span>
							</p>
						</v-flex>
					</v-layout>
			...
			<v-layout row wrap>
				<v-flex xs12 md10 sm10>
					<v-layout row wrap justify-end>
						<v-flex xs5 md3 sm3 d-flex> 
							<v-select :items="items2" label="All" solo background-color="#FCD675" v-on:change="OnSelectOptionChange" v-model="defaulSelecttVal">
							</v-select>
						</v-flex>
					</v-layout>
				</v-flex>
				...
				<template v-if="showBookings.length > 0">
				  <div v-for="(showBooking,ind) in showBookings" :key="ind">
					<router-link v-if="completedBooking" style="text-decoration:none" 
					:to="{ name: 'BookingsClient', params: { completed:'complete' , clientId :showBooking.client_id , date:showBooking.date , time:showBooking.time , category:showBooking.service_name , barber:showBooking.barber_name , price:showBooking.payment , barberId:showBooking.barber_id } }">
						<v-layout row wrap justify-center>
							<v-flex xs2 md2 sm2>
								<span class="text">
								   {{ showBooking.date }}
				...
				<template v-if="showBookingsOne.length > 0">
					<div v-for="(showBookingOne,inde) in showBookingsOne" :key="inde">
							<router-link v-if="pendingBooking" style="text-decoration:none" 
				     	:to="{ name: 'BookingsClient', params: { completed:'pending' , clientId :showBookingOne.client_id , date:showBookingOne.date , time:showBookingOne.time , category:showBookingOne.service_name , barber:showBookingOne.barber_name , price:showBookingOne.payment , barberId:showBookingOne.barber_id } }">
				...
				<template v-if="showBookings2.length > 0">
					...
					<router-link v-if="canceledBooking" style="text-decoration:none" 
						:to="{ name: 'BookingsClient', params: { completed:'cancel' , clientId :showBooking2.client_id , date:showBooking2.date , time:showBooking2.time , category:showBooking2.service_name , barber:showBooking2.barber_name , price:showBooking2.payment , barberId:showBooking2.barber_id } }">
						<v-layout row wrap justify-center mt-2>
							<v-flex xs2 md2 sm2>
								<span class="text">
									{{ showBooking2.date }}
...
import moment from 'moment'
      ...
      data () {
        return {
            ...
            showBookingsOne:[],
						showBookings2:[],
						CalenderBookings:[],
            items2: ['All', 'Completed', 'Pending','Canceled']
        }
			},
			computed: {
    ...
    beforeMount(){
      ...
      methods:{
			   ...
			   this.menu1 = false
				 this.loader = true
					...
					this.$store.dispatch('showCalenderBookings', {
      ...
      mounted (){
		    this.loader = true
        this.$store.dispatch('showBooking', {
                user_id: this.$store.getters.loggedInUserId,
              }) 
            .then(response => {
              this.loader = false
        ...
        if(this.$route.query.complete)
        {
          ...
          this.completedBooking = true
          this.pendingBooking = false
          this.defaulSelecttVal = "Completed"
        }
